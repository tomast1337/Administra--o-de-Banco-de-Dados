...
select cliente.nome, agencia.nome, conta.saldo
...
select cliente.nome, count(emprestimo.id)
...
select 
    cliente.nome, agencia.nome, cliente.cidade
from cliente, agencia, conta, emprestimo
where cliente.cpf = conta.cpf_cliente
and agencia.id = conta.id_agencia
and cliente.cpf = emprestimo.cpf_cliente
and cliente.cpf not in (select cpf_cliente from conta); -- ERRADO ?
...
select agencia.nome, cliente.nome
from agencia, cliente
where agencia.id = cliente.id_agencia;
